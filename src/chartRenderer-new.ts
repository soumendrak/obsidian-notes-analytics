...
export class ChartRenderer {
	...
	renderBarChart(data: { label: string; value: number }[], title: string, color: string = '#007acc') {
		...
		data.forEach((point, i) => {
			...
			// Store data point for tooltip (center of bar)
	...
	renderAreaChart(data: { label: string; value: number }[], title: string, color: string = '#007acc') {
		...
		if (data.length === 1) {
			...
			// Store data point for tooltip
			this.dataPoints.push({ x: x + 10, y: y + barHeight / 2, label: data[0].label, value: data[0].value });
			return;
		}
	...
	renderPieChart(data: { label: string; value: number }[], title: string, colors: string[] = ['#007acc', '#28a745', '#dc3545', '#ffc107', '#6f42c1', '#fd7e14']) {
		...
		data.forEach((item, i) => {
			...
			// Store data point for tooltip (middle of slice)
			const midAngle = currentAngle + sliceAngle / 2;
			const tooltipX = centerX + Math.cos(midAngle) * (radius * 0.7);
			const tooltipY = centerY + Math.sin(midAngle) * (radius * 0.7);
			this.dataPoints.push({ x: tooltipX, y: tooltipY, label: item.label, value: item.value });
